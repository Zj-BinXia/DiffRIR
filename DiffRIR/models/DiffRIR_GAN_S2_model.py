...
from DiffRIR.utils.img_util import add_lumi_error

@MODEL_REGISTRY.register()
class DiffRIRGANS2Model(SRGANModel):
    """
    ...
    def __init__(self, opt):
        super(DiffRIRGANS2Model, self).__init__(opt)
    ...
    def setup_optimizers(self):
        ...
        for k,v in self.net_g.named_parameters():
            if "denoise" in k or "condition" in k:
                parms.append(v)
        self.optimizer_e = self.get_optimizer(optim_type_g, parms, **train_opt['optim_g'])
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('kd_opt'):
            ...
            self.cri_kd = None
        super(DiffRIRGANS2Model, self).init_training_settings()
    ...
    def feed_data(self, data):
        ...
        if self.is_train and self.opt.get('high_order_degradation', True):
            ...
            #out=add_lumi_error(out,data['Y_cof'].to(self.device),data['Y_bias'].to(self.device))
            ...
            (self.gt, self.gt_usm,self.mask), self.lq = paired_random_crop([self.gt, self.gt_usm,self.mask], self.lq, gt_size,
                                                                 self.opt['scale'])
            ...
            if self.opt['scale']==4:
                ...
                self.lq = torch.cat([self.lq, gt],dim=1)
            elif self.opt['scale']==2:
                gt=(1-self.mask)*gt
                gt=self.pixel_unshufflev2(gt)
                self.lq = torch.cat([self.lq, gt],dim=1)
            else:
                self.lq = torch.cat([self.lq, (1-self.mask)*gt],dim=1)
        else:
            ...
            if self.opt['scale']==4:
                ...
                self.lq = torch.cat([self.lq, (1-self.mask)*self.gt],dim=1)

    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        # do not use the synthetic process during validation
        self.is_train = False
        super(DiffRIRGANS2Model, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
    ...
    def test(self):
        ...
        if self.opt['scale']==1:
            self.output=(1-self.mask)*self.gt + self.output*self.mask
    ...
    def optimize_parameters(self, current_iter):
        ...
        # if self.opt['scale']==1:
        #     self.output=(1-self.mask)*l1_gt + self.output*self.mask
        l_g_total = 0
