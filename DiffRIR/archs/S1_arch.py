import DiffRIR.archs.common as common
import torch
...
class DIRformer(nn.Module):
    ...
    def forward(self, inp_img,k_v):
        ...
        out_dec_level1 = self.tail(out_dec_level1) + F.interpolate(inp_img[:,:3,:,:], scale_factor=self.scale, mode='nearest') 
...
class CPEN(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6,scale=4):
        ...
        self.scale=scale
        if scale == 2:
            E1=[nn.Conv2d(108, n_feats, kernel_size=3, padding=1),
                nn.LeakyReLU(0.1, True)]
        elif scale == 1:
            E1=[nn.Conv2d(144, n_feats, kernel_size=3, padding=1),
                nn.LeakyReLU(0.1, True)]
        else:
            E1=[nn.Conv2d(99, n_feats, kernel_size=3, padding=1),
                nn.LeakyReLU(0.1, True)]
        E2=[
            common.ResBlock(
