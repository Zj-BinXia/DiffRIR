...
if __name__ == '__main__':
    ...
    gen_mask=get_mask_generator(mask_kind,mask_kwargs)
    scale=4
    lr="./Test/%d/LR"%(scale)
    hr="./Test/%d/HR"%(scale)
    mask="./Test/%d/MASK"%(scale)
    lrs=os.listdir(lr)
    for idx,f in enumerate(lrs):
        lr2= os.path.join(lr,f)
        lr_img=cv2.imread(lr2)
        mask=gen_mask(lr,idx)
        print(lr_img.shape,mask.shape)
